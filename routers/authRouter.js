...
router.post('/register', uploader.single('photo'), registerRules, async (req, res, next) => {
  ...
  // 還沒雜湊的密碼上傳
  let result = await pool.execute('INSERT INTO users (users_account, users_password, users_name, user_imageHead) VALUES (?, ?, ?, ?);', [req.body.account, req.body.password, req.body.name]);
